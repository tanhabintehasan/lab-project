-- Migration: ServiceCategory flat columns → JSONB (final cleanup)
-- Idempotent, preserves all existing data.

-- 1. Add deletedAt if missing (required by soft-delete extension)
ALTER TABLE "ServiceCategory"
  ADD COLUMN IF NOT EXISTS "deletedAt" TIMESTAMP(3);

-- 2. Add new JSONB columns if not already present
ALTER TABLE "ServiceCategory"
  ADD COLUMN IF NOT EXISTS "name" JSONB,
  ADD COLUMN IF NOT EXISTS "description" JSONB,
  ADD COLUMN IF NOT EXISTS "seoTitle" JSONB,
  ADD COLUMN IF NOT EXISTS "seoDescription" JSONB;

-- 3. Migrate data from old flat columns to JSONB (idempotent)
UPDATE "ServiceCategory"
SET
  "name" = jsonb_build_object(
    'zh', COALESCE("nameZh", ''),
    'en', "nameEn"
  ),
  "description" = jsonb_build_object(
    'zh', "descZh",
    'en', "descEn"
  ),
  "seoTitle" = jsonb_build_object(
    'zh', "seoTitleZh",
    'en', "seoTitleEn"
  ),
  "seoDescription" = jsonb_build_object(
    'zh', "seoDescZh",
    'en', "seoDescEn"
  )
WHERE "name" IS NULL;

-- 4. Ensure "name" is NOT NULL (matches Prisma schema)
ALTER TABLE "ServiceCategory"
  ALTER COLUMN "name" SET NOT NULL;

-- 5. Add self-referencing foreign key if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'ServiceCategory_parentId_fkey'
    AND table_name = 'ServiceCategory'
  ) THEN
    ALTER TABLE "ServiceCategory"
      ADD CONSTRAINT "ServiceCategory_parentId_fkey"
      FOREIGN KEY ("parentId") REFERENCES "ServiceCategory"(id) ON UPDATE NO ACTION ON DELETE SET NULL;
  END IF;
END $$;

-- 6. Drop old flat columns
ALTER TABLE "ServiceCategory"
  DROP COLUMN IF EXISTS "nameZh",
  DROP COLUMN IF EXISTS "nameEn",
  DROP COLUMN IF EXISTS "descZh",
  DROP COLUMN IF EXISTS "descEn",
  DROP COLUMN IF EXISTS "seoTitleZh",
  DROP COLUMN IF EXISTS "seoTitleEn",
  DROP COLUMN IF EXISTS "seoDescZh",
  DROP COLUMN IF EXISTS "seoDescEn";
