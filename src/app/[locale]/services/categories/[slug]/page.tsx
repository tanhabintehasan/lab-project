        ...
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <ServiceCardSkeleton key={`category-skeleton-${i}`} />
            ))}
          </div>
        ) : services.length === 0 ? (
          <p className="text-center py-16 text-gray-500">{t('noResults')}</p>
        ) : (
          <>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.map((service) => {
                ...
                const nameZh = service.nameZh || t('unnamedService');
