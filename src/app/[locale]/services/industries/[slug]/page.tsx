...
export default function IndustryServicesPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`/api/services?industry=${slug}&page=${page}&pageSize=12`, {
      credentials: 'include',
    })
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-8">行业: {slug}</h1>

        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <ServiceCardSkeleton key={`industry-skeleton-${i}`} />
            ))}
          </div>
        ) : services.length === 0 ? (
