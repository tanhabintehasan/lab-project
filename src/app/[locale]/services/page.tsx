...
function formatPrice(value: number | null, t: any): string {
  if (value === null || Number.isNaN(value)) return t('inquire');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function normalizeService(item: ApiService, t: any): NormalizedService {
  ...
  return {
    ...
    name: item.nameZh || t('unnamedService'),
    category: item.category?.nameZh || t('uncategorized'),
    description:
      item.shortDescZh ||
      item.description ||
      t('noResultsDesc'),
    price: priceValue,
    priceUnit: item.priceUnit || t('priceUnit'),
    turnaround: item.turnaround || (days > 0 ? t('turnaroundDays', { days }) : t('turnaroundDefault')),
...
export default function ServicesPage() {
  ...
  const categoryOptions = useMemo(() => {
    return [
      { id: 'all', name: t('allCategories'), count: undefined as number | undefined, value: 'all' },
      ...categories.map((cat) => ({
        id: cat.id,
        value: cat.slug || cat.id,
        name: cat.nameZh || t('unnamedService'),
  ...
  const fetchServices = useCallback(async () => {
    try {
      ...
      const items = extractArray<ApiService>(data).map((item) => normalizeService(item, t));
        ...
        <div className="flex gap-8">
          <aside className="hidden lg:block w-64 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
              ...
              <div className="mb-6">
                <h4 className="text-sm font-medium text-gray-700 mb-3">{t('serviceCategory')}</h4>
                ...
                {categoryLoading && (
                  <p className="text-xs text-gray-400 mt-3">{t('loadingCategories')}</p>
                )}
              </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <p className="text-sm text-gray-500">
                {t('resultsCount', { count: apiServices.length })}
              </p>
                    ...
                    >
                      ...
                      <div className={viewMode === 'list' ? 'flex-1 min-w-0' : 'p-5'}>
                        ...
                        <p className="text-xs text-gray-400 mt-2">
                          {t('servedNTimes', { count: service.orderCount })}
                        </p>
                      </div>
                    </Link>
                  ))}
