...
export async function GET(request: NextRequest) {
  try {
    ...
    // --- DB connectivity / schema sanity check ---
    try {
      await prisma.$queryRaw`SELECT 1`;
    } catch (connErr: any) {
      console.error('[API /services] Database connection failed:', connErr);
    ...
    // --- Main query with isolated error handling ---
    ...
    try {
      ...
      ]) as [ServiceRow[], number];
    } catch (prismaErr: any) {
      console.error('[API /services] Prisma query failed:', prismaErr);
      return errorResponse(
        `Service query failed: ${prismaErr?.message || 'Unknown Prisma error'}`,
        500
      );
    }

    const data = services.map((service: ServiceRow) => ({
      id: service.id,
      slug: service.slug,
    ...
    console.error('[API /services] Unhandled exception:', error);
    return errorResponse(
