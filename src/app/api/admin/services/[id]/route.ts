  ...
  try {
    ...
    const updateData: Prisma.TestingServiceUpdateInput = {
      ...
      ...(data.sampleRequirement !== undefined
        ...
        : {}),
      ...(data.sampleCount !== undefined ? { sampleCount: data.sampleCount?.trim() || null } : {}),
      ...(data.sampleSize !== undefined ? { sampleSize: data.sampleSize?.trim() || null } : {}),
      ...(data.sampleWeight !== undefined ? { sampleWeight: data.sampleWeight?.trim() || null } : {}),
      ...(data.sampleCondition !== undefined ? { sampleCondition: data.sampleCondition?.trim() || null } : {}),
      ...(data.samplePreservation !== undefined ? { samplePreservation: data.samplePreservation?.trim() || null } : {}),
      ...(data.samplePreparation !== undefined ? { samplePreparation: data.samplePreparation?.trim() || null } : {}),
      ...(data.isFeatured !== undefined ? { isFeatured: data.isFeatured } : {}),
      ...(data.isHot !== undefined ? { isHot: data.isHot } : {}),
    };

    // Handle customFields update: delete existing and recreate if provided
    if (data.customFields !== undefined) {
      await prisma.testingServiceCustomField.deleteMany({
        where: { serviceId: existing.id },
      });
      if (data.customFields.length > 0) {
        await prisma.testingServiceCustomField.createMany({
          data: data.customFields.map((f) => ({
