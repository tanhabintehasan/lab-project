...
import {
  LayoutDashboard, Users, Wallet, ShoppingCart, FileText,
  CheckCircle2, Building2, ChevronLeft, ChevronRight, Bell, Menu,
} from 'lucide-react';
...
export function EnterpriseLayout({ children }: EnterpriseLayoutProps) {
  ...
  const menuItems = [
    { href: '/enterprise/workspace', icon: LayoutDashboard, label: t('workspace') },
    { href: '/enterprise/members', icon: Users, label: t('members') },
    { href: '/enterprise/billing', icon: Wallet, label: t('billing') },
    { href: '/enterprise/orders', icon: ShoppingCart, label: '企业订单' },
    { href: '/enterprise/reports', icon: FileText, label: '企业报告' },
    { href: '/enterprise/approvals', icon: CheckCircle2, label: t('approvals') },
  ];

  const renderSidebar = () => (
    <aside className={cn('bg-teal-900 text-teal-200 flex flex-col h-full transition-all duration-200', collapsed ? 'w-16' : 'w-60')}>
