...
export function verificationEmail(data: { name: string; code: string; verifyUrl: string }): { subject: string; html: string; text: string } {
  const html = baseTemplate(`
    <h2>验证您的邮箱</h2>
    <p>您好 ${data.name}，</p>
    <p>感谢您注册 ${siteName}。请使用以下验证码完成邮箱验证：</p>
    <p style="text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 4px; color: #667eea; margin: 30px 0;">
      ${data.code}
    </p>
    <p style="text-align: center;">
      <a href="${data.verifyUrl}" class="button">立即验证</a>
    </p>
    <p>或复制以下链接到浏览器：</p>
    <p style="word-break: break-all; color: #667eea;">${data.verifyUrl}</p>
    <div class="divider"></div>
    <p style="color: #999; font-size: 14px;">此验证码将在15分钟后失效。如果您没有注册账号，请忽略此邮件。</p>
  `);

  const text = `验证您的邮箱\n\n您好 ${data.name}，\n\n感谢您注册 ${siteName}。请使用以下验证码完成邮箱验证：\n\n${data.code}\n\n立即验证：${data.verifyUrl}\n\n此验证码将在15分钟后失效。如果您没有注册账号，请忽略此邮件。`;

  return { subject: `验证您的邮箱 - ${siteName}`, html, text };
}
