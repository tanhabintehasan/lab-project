    ...
    if (config.type === 'SMS') {
      ...
      try {
        ...
        const testEmailResult = await sendEmail({
          to: config.endpoint || 'admin@labtest.com',
          subject: '邮件服务测试',
          html: '<p>这是一封测试邮件，用于确认邮件服务配置正确。</p>',
          text: '这是一封测试邮件，用于确认邮件服务配置正确。',
        });
        if (testEmailResult.success) {
          testResult = { success: true, message: `Test email sent. Message ID: ${testEmailResult.messageId || 'N/A'}` };
        } else {
          testResult = { success: false, message: testEmailResult.error || 'Failed to send test email' };
        }
      } catch (err) {
        testResult = { success: false, message: err instanceof Error ? err.message : 'Email test failed' };
      }
    } else {
